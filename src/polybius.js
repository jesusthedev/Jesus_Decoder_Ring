...
const polybiusModule = (function () {
  ...
  function polybius(input, encode = true) {
    //create polybius square collection
    ...
    const message = input.toLowerCase().split(''); //our input -> individual array
    const messageNoSpaces = message.filter(nums => nums != ' '); //array  without spaces
    if(!encode){ //then decode
      let decodeString = '';
      if(messageNoSpaces.length % 2 != 0) return false; // copy of array without spaces to check if there are an odd number
      for(let index = 0; index < message.length; index += 2){
        //[column][row] format
        if(message[index] === ' '){ //if a space in the input add it to our decodeString
          decodeString += ' ';
